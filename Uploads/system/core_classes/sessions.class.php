...
class OHSNAPSessions {
    ...
    public    $SNAP;
    protected $_session_duration    = 15;
    protected $_session_ip_matching = 0;
    protected $_session_lifetime    = 0;
    
    public function manage_sessions()
    {
        $this->_session_duration    = $this->SNAP->CFG['session_timeout'];
        $this->_session_duration    = ( $this->_session_duration * 60 );
        $this->_session_ip_matching = $this->SNAP->CFG['session_ip_matching'];
        ...
        if ( isset( $_COOKIE['ohsnap_token'] ) )
        {
            ...
            switch ( $found )
            {
                ...
                $r     = $this->SNAP->DB->db_fetch_object( $sql );
                ...
                switch ( $total )
                {
                    ...
                    switch ( $this->_session_ip_matching )
                    {
                        ...
                        if ( ( $r->ip_address != $this->SNAP->AGENT['ip'] ) OR ( $r->user_agent != $this->SNAP->AGENT['agent'] ) )
                        {
                            ...
                            {
                                $this->SNAP->delete_cookie( session_name(), true );
                            }
                        ...
                        {
                            $this->SNAP->SESSION['expires']     = ( time() + $this->_session_duration );
                            $this->SNAP->SESSION['last_click']  = time();
                    ...
                    switch ( $_COOKIE['ohsnap_anonymous'] )
                    {
                        case true:
                        $this->SNAP->MEMBER['anonymous'] = 1;
                        break;
                        
                        case false:
                        $this->SNAP->MEMBER['anonymous'] = 0;
                        break;
                    }
        ...
        else
        {
            ...
            switch ( $total )
            {
                ...
                case 0:
                $this->SNAP->MEMBER['id']             = 0;
                $this->SNAP->MEMBER['username']       = 'Guest';
                $this->SNAP->MEMBER['display_name']   = 'Guest';
                $this->SNAP->SESSION['expires']       = ( time() + $this->_session_duration );
                $this->SNAP->SESSION['last_click']    = time();
                $this->SNAP->SESSION['location']      = $this->SNAP->fetch_server_var( 'REQUEST_URI' );
                $this->SNAP->MEMBER['anonymous']      = 0;
    ...
    {
        $this->SNAP->DB->db_query( $this->SNAP->SQL->sql_insert_session_new() );
    }
    
    public function update_user_session()
    {
        $this->SNAP->DB->db_query( $this->SNAP->SQL->sql_update_session() );
    }
    ...
    {
        $this->SNAP->TOSQL = array( 'time' => time() );
        
        $this->SNAP->DB->db_query( $this->SNAP->SQL->sql_delete_session_gc() );
    }
    ...
    public function session_read( $id )
    {
        ...
        if ( $this->SNAP->DB->db_num_rows( $sql ) > 0 )
        {
            $r    = $this->SNAP->DB->db_fetch_array( $sql );
            $data = $r['data'];
        }
    ...
    public function session_destroy( $id )
    {
        ...
        $this->SNAP->DB->db_query( $this->SNAP->SQL->sql_delete_session_store() );
    }
    
    public function session_garbage_collection()
    {
        $this->SNAP->DB->db_query( $this->SNAP->SQL->sql_delete_session_store_gc() );
    }
}
